...
def miller_rabin_deterministic_variant(n, variant_config):
    ...
    if n <= 1:
        return "composite"
    if n == 2 or n == 3:
        return "prime"  # All variants should classify these as prime
    if n % 2 == 0:
        return "composite"
    ...
    for b in bases_to_test:
        if (
            b < n and n % b == 0
        ):  # If n is divisible by one of its prime bases (and not the base itself)
            return "composite"
    ...
    for a in bases_to_test:
        if n == a:
            continue  # Already handled if n is a base
...
def run_benchmark(benchmark_test_limit=10000):  # Reduced for quicker demonstration
    ...
    for variant_config in DETERMINISTIC_VARIANTS:
        ...
        for num in numbers_to_test:
            ...
            if (
                num < variant_config["limit"]
            ):  # Only test if num is within this variant's deterministic range
        ...
        benchmark_results_list.append(
            {
                ...
                "N Benchmarked Up To": benchmark_test_limit
                - 1,  # The common range chosen for benchmark
...
if __name__ == "__main__":
    ...
    print(
        "Ensure 'pandas' and 'openpyxl' (for Excel) are installed: pip install pandas openpyxl matplotlib"
    )
    ...
    for num in example_numbers_to_test:
        ...
        for variant_config in DETERMINISTIC_VARIANTS:
            ...
            if (
                num < variant_config["limit"] or num < 5000
            ):  # Show for smaller numbers, or if clearly in range
    ...
    df_example_results = pd.DataFrame(
        example_results_data[1:], columns=example_results_data[0]
    )
    ...
    try:
        with pd.ExcelWriter(excel_filename, engine="openpyxl") as writer:
            df_example_results.to_excel(
                writer, sheet_name="Variant Examples", index=False
            )
            df_benchmark_results.to_excel(
                writer, sheet_name="Benchmark Comparison", index=False
            )
    ...
    bars = ax.bar(
        graph_labels, graph_times, color=plt.cm.viridis(range(len(graph_labels)))
    )

    ax.set_ylabel("Time Taken (seconds)")
    ax.set_xlabel("Miller-Rabin Test Variant")
    ax.set_title(
        f"Miller-Rabin Performance (Numbers up to {BENCHMARK_UPPER_BOUND_N-1})"
    )
