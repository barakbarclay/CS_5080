...
def calculate_empirical_rate_two_primes(K_adversary, alice_prime_pool, num_trials=10000):
    ...
    for _ in range(num_trials):
        ...
        if (y % p1_alice == x % p1_alice) and \
           (y % p2_alice == x % p2_alice):
            false_positives += 1
    return false_positives / num_trials

def calculate_empirical_rate_three_primes(K_adversary, alice_prime_pool, num_trials=10000):
    ...
    for _ in range(num_trials):
        ...
        if (y % p1_alice == x % p1_alice) and \
           (y % p2_alice == x % p2_alice) and \
           (y % p3_alice == x % p3_alice):
            false_positives += 1
    return false_positives / num_trials
...
def run_adaptive_adversary_experiment(n_start=6, n_end=100, num_trials_empirical=10000):
    ...
    for n in range(n_start, n_end + 1):
        ...
        K_orig, num_factors_K_orig = calculate_k_for_adversary(n, alice_pool_orig, "OriginalPool")
        ...
        emp_rate_cm4 = calculate_empirical_rate_three_primes(K_restr, alice_pool_restricted, num_trials_empirical)
...
def create_adaptive_interactive_plot(results, filename="fingerprint_adaptive_adv_plot_v2.html"):
    ...
    fig.add_trace(go.Scatter(
        x=results['n_values'], y=results['theo_rate_orig_baseline'], mode='lines', name='Theo. Rate (Orig Baseline)',
        line=dict(dash='solid', color='grey', width=1.5),
        hovertemplate='n=%{x}<br>Theo Rate (Orig Baseline)=%{y:.6f}<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=results['n_values'], y=results['emp_rate_orig'], mode='lines+markers', name='Emp. Orig (1 prime (n,n²), K_orig)',
        marker=dict(symbol='x', size=6, color='blue'), line=dict(dash='dash', color='blue'),
        hovertemplate='n=%{x}<br>Emp Rate (Orig)=%{y:.6f}<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=results['n_values'], y=results['emp_rate_cm1'], mode='lines+markers', name='Emp. CM1 (2 primes (n,n²), K_orig)',
    ...
    fig.add_trace(go.Scatter(
        x=results['n_values'], y=results['emp_rate_cm2'], mode='lines+markers', name='Emp. CM2 (1 prime (nln n,n²), K_restr)',
    ...
    fig.add_trace(go.Scatter(
        x=results['n_values'], y=results['emp_rate_cm3'], mode='lines+markers', name='Emp. CM3 (2 primes (nln n,n²), K_restr)',
    ...
    fig.add_trace(go.Scatter(
        x=results['n_values'], y=results['emp_rate_cm4'], mode='lines+markers', name='Emp. CM4 (3 primes (nln n,n²), K_restr)',
        marker=dict(symbol='hexagon', size=6, color='orange'), line=dict(dash='solid', width=1.5, color='orange'),
