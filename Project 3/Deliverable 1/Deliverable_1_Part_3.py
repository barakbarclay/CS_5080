...
def calculate_adversary_k_and_theoretical_rate(n, primes_between_n_n2):
    ...
    for p in sorted_primes:
        ...
        if p <= 0:
            continue  # Should not happen for primes > n
...
def run_comparison_experiment(n_start=6, n_end=200, num_trials_empirical=10000):
    ...
    for n in range(n_start, n_end + 1):
        if n <= 1:
            continue  # Skip n=0, 1 as log(n) is problematic
        ...
        emp_rate_cm2 = (
            calculate_empirical_rate_restricted_primes(
                K, primes_restricted, num_trials=num_trials_empirical
            )
            if can_run_cm2
            else float("nan")
        )
        ...
        if n == 6 or n == 10 or n % 20 == 0 or n == n_end:
            ...
            print(
                f"n={n}: Primes(orig)={len(primes_n_n2)}, Primes(restr)={len(primes_restricted)}, Adv Primes={num_adv_primes}, Theo Rate={theo_rate:.4f}, Emp Orig={emp_rate_orig:.4f}, Emp CM1={cm1_str}, Emp CM2={cm2_str}"
            )
...
def create_interactive_plot(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=theoretical_rates,
            mode="lines",
            name="Theoretical Rate (Original Baseline)",
    ...
    fig.add_trace(
        go.Scatter(
            x=n_values,
            y=empirical_rates_cm1,
            mode="lines+markers",
            name="Empirical Rate (CM1: Two Prints)",
    ...
    fig.add_trace(
        go.Scatter(
            x=n_values,
            y=empirical_rates_cm2,
            mode="lines+markers",
            name="Empirical Rate (CM2: Restricted Primes p>n*ln(n))",
            marker=dict(symbol="star", size=5, color="green"),
...
if __name__ == "__main__":
    ...
    n_vals, theo_rates, emp_rates_o, emp_rates_c1, emp_rates_c2 = (
