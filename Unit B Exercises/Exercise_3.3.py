    ...
    for _ in range(len(node_order) - 1):
        ...
        for remaining_node in temp_graph2.nodes():
            if remaining_node != remaining_node_order[0]:
                ...
                remaining_shortcut_scores[remaining_node] = contract_node(
                    temp_graph3, remaining_node
                )
                shortcut_scores[remaining_node] = remaining_shortcut_scores[
                    remaining_node
                ]
        remaining_node_order = sorted(
            remaining_shortcut_scores, key=remaining_shortcut_scores.get
        )
