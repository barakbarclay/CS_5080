...
def bidirectional_dijkstra(graph, source, target, node_order_map):
    ...
    while forward_queue and backward_queue:
        ...
        for neighbor, edge_data in sorted(graph[forward_node].items(), key=lambda x: node_order_map[x[0]]):
        ...
        for neighbor, edge_data in sorted(graph[backward_node].items(), key=lambda x: node_order_map[x[0]]):
