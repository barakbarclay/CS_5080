        ...
        for size in sizes:
            ...
            for t in range(trials):
                ...
                try:
                    ...
                    comps, time_taken, _, _ = quicksort(arr_copy_for_sort, pivot_strategy=pivot_strategy_to_use)
                    ...
                    print(f"      ERROR occurred during quicksort: {e}. Skipping trial.")
                    times.append(float('nan'))
                    comparisons_list.append(float('nan'))


            avg_time = np.nanmean(times) if not np.all(np.isnan(times)) else float('nan')
            avg_comparisons = np.nanmean(comparisons_list) if not np.all(np.isnan(comparisons_list)) else float('nan')
...
for strategy in pivot_strategies:
    ...
    for dist in distributions:
        df_subset = df_results[df_results['Distribution'] == dist]
        plt.plot(df_subset['Size'], df_subset['AverageComparisons'], marker='x', linestyle='--', label=dist)
    plt.xlabel("Input Size (n)")
