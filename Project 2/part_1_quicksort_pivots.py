...
def quicksort(arr, pivot_strategy="last"):
    ...
    quicksort_recursive(
        arr_copy, 0, len(arr_copy) - 1, pivot_strategy, comparison_counter
    )
...
def run_experiment():
    ...
    print(
        "{:<12} {:<20} {:<15} {:<15}".format(
            "Size", "Pivot Strategy", "Avg Compares", "Avg Time (s)"
        )
    )
    print("-" * 60)
        ...
        try:
            ...
            for strategy in pivot_strategies:
                ...
                for run in range(num_runs):
                    ...
                    try:
                        comparisons, exec_time = quicksort(
                        ...
                        total_comparisons = float(
                            "nan"
                        )  # Mark average as NaN if any run fails
                ...
                if num_runs > 0 and not (
                    isinstance(total_comparisons, float)
                    and total_comparisons != total_comparisons
                ):  # Check if not NaN
                    avg_comparisons = total_comparisons / num_runs
                    avg_time = total_time / num_runs
                    print(
                        "{:<12} {:<20} {:<15.1f} {:<15.6f}".format(
                            size, strategy, avg_comparisons, avg_time
                        )
                    )
                else:
                    print(
                        "{:<12} {:<20} {:<15} {:<15}".format(
                            size, strategy, "N/A (Failed)", "N/A (Failed)"
                        )
                    )
