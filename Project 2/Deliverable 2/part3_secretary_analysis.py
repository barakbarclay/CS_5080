...
def generate_candidates_variable_uniform(n):
  ...
  np.random.shuffle(all_scores) # Shuffle to simulate random arrival from the mixed pool
...
def run_classical_strategy(candidates, k, estimator_func):
    ...
    if k >= n or k < 0: # Handle edge cases for k
         ...
         if k > 0 and n > 0:
             # We still need to see the first k to potentially estimate later
              max_seen_in_rejection = np.max(candidates[:min(k, n)]) if min(k, n) > 0 else -1
    ...
    estimates = []
    max_seen_so_far = max_seen_in_rejection # Needed for estimator after rejection phase
    ...
    for i in range(k, n):
        ...
        current_max_seen = np.max(candidates[:i+1]) # Max including the current candidate
        ...
        if current_candidate_score > best_seen_before_current:
            ...
            selected_index = i
            estimate_at_selection = current_estimate # Use the estimate calculated at this step
    ...
    elif not estimates and n > 0: # e.g., k=n case handled above, but maybe k=n-1, loop runs once?
...
def advanced_estimator(max_seen_so_far, t, n):
    ...
    for i in range(1, n - t + 1):
         ...
         if t + i == 0: continue # Should not happen given t>=1 check
         sum_term += 1.0 / (t + i)
...
if __name__ == "__main__":
    ...
    print(f"Parameters: n = {N_CANDIDATES}, k = {K_THRESHOLD} (reject first {K_THRESHOLD}), Trials = {N_TRIALS}")
    print("-" * 30)
    ...
    if results_uniform_basic['avg_estimator_diff'] is not None:
       print(f"  Avg. Estimator Difference (Estimate - True Best, when selected): {results_uniform_basic['avg_estimator_diff']:.4f}")
    else:
       print(f"  Avg. Estimator Difference: Not applicable (no selections made or estimator failed).")
    print("-" * 30)
    ...
    if results_variable_basic['avg_estimator_diff'] is not None:
       print(f"  Avg. Estimator Difference (Estimate - True Best, when selected): {results_variable_basic['avg_estimator_diff']:.4f}")
    else:
    ...
    print(f"  Change in Success Rate: {results_variable_basic['success_rate'] - results_uniform_basic['success_rate']:.4f}")
    print(f"  Change in Avg. Estimator Diff: {(results_variable_basic['avg_estimator_diff'] or 0) - (results_uniform_basic['avg_estimator_diff'] or 0):.4f}")
    print("  Discussion:")
    ...
    print("  - If k is large (e.g., k > n/2), the rejection phase sees both distributions,")
    ...
    print("  The basic estimator's performance (simply using max seen) might also change;")
    ...
    if results_uniform_advanced['avg_estimator_diff'] is not None:
       print(f"  Avg. Estimator Difference (Estimate - True Best, when selected): {results_uniform_advanced['avg_estimator_diff']:.4f}")
    else:
       print(f"  Avg. Estimator Difference: Not applicable (no selections made).")
    print("-" * 30)
    ...
    print(f"  Basic Estimator Avg. Difference:   {results_uniform_basic['avg_estimator_diff']:.4f}")
    print(f"  Advanced Estimator Avg. Difference: {results_uniform_advanced['avg_estimator_diff']:.4f}")
    print("  Discussion:")
    ...
    print("  using the current maximum (Basic Estimator), especially earlier in the process.")
    print("  We expect the Advanced Estimator's average difference to be closer to zero,")
